...
local UPDATE_PATH = "/Maauh/BoL/Thresh/ThreshThePower.lua".."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH..GetCurrentEnv().FILE_NAME
local UPDATE_URL = "https://"..UPDATE_HOST..UPDATE_PATH

function _AutoupdaterMsg(msg) print("<font color=\"#6699ff\"><b>Thresh:</b></font> <font color=\"#FFFFFF\">"..msg..".</font>") end
if AUTOUPDATE then
	local ServerData = GetWebResult(UPDATE_HOST, "/Maauh/BoL/Thresh/Thresh.version")
	if ServerData then
...
if VIP_USER and FileExist(LIB_PATH .. "/DivinePred.lua") then 
	require "DivinePred" 
	dp = DivinePred()
	local skillshotQ = LineSS(1850,1150, 140, 50, 0,hero_HIT, HUGE)
	local skillshotQ = dp:bindSS("ThreshQ",skillshotQ,50)
...
spells = {}
spells.q = {name = myHero:GetSpellData(_Q).name, ready = false, range = 1150, width = 140}
spells.w = {name = myHero:GetSpellData(_W).name, ready = false, range = 950, width = nil}
spells.e = {name = myHero:GetSpellData(_E).name, ready = false, range = 520, width = nil}
	...
	if ValidTarget(unit) and GetDistance(unit) <= settings.combo.qRange and myHero:GetSpellData(_Q).name ~= "threshqleap" then
		if (unit.charName == Champ[1] and settings.q.champ1) or (unit.charName == Champ[2] and settings.q.champ2) or (unit.charName == Champ[3] and settings.q.champ3) or (unit.charName == Champ[4] and settings.q.champ4) or (unit.charName == Champ[5] and settings.q.champ5) then
			...
			elseif settings.pred == 2 and VIP_USER and os.clock() - qLast > 0.2 then
				...
				local state,hitPos,perc = dp:predict("ThreshQ",target)
				if hitPos then CastSpell(_Q, hitPos.x, hitPos.z)
				end
			elseif settings.pred == 3 then
				  local QPos, QHitChance = HPred:GetPredict(HP_Q, unit, myHero) 
				 if QHitChance >= 1.2 then -- QPos can't be nil in HPred when QHitChance ~= 0
        CastSpell(_Q, QPos.x, QPos.z)
         end
			elseif settings.pred == 4 then
...
function OnLoad()
	if not loaded then
		...
		if hpload then
			HP_Q = HPSkillshot({type = "DelayLine", delay = 0.5, range = 1150, width = 150, speed=1850})
		end
	end
...
function Menu()
    ...
    settings:addParam("pred", "Prediction Type", SCRIPT_PARAM_LIST, 1, { "VPrediction", "DivinePred", "HPred", "FHPrediction"})
end
